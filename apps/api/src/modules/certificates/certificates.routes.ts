...
const schemas = {
  certificates: z.object({ studentName: z.string().min(2), certificateNumber: z.string().min(2), title: z.string().min(2), issuedOn: z.coerce.date(), fileUrl: z.string().min(2), status: z.string().default("ISSUED") }),
  transcripts: z.object({ studentName: z.string().min(2), transcriptNumber: z.string().min(2), academicYear: z.string().min(2), gpa: z.string().min(1), fileUrl: z.string().min(2), status: z.string().default("PUBLISHED") }),
  verifications: z.object({ verificationCode: z.string().min(2), certificateNumber: z.string().min(2), studentName: z.string().min(2), verifiedOn: z.coerce.date().optional(), status: z.string().default("VALID") })
} as const;
...
const columnsByResource: Record<Resource, string[]> = {
  certificates: ["id", "studentName", "certificateNumber", "title", "issuedOn", "fileUrl", "status"],
  transcripts: ["id", "studentName", "transcriptNumber", "academicYear", "gpa", "fileUrl", "status"],
  verifications: ["id", "verificationCode", "certificateNumber", "studentName", "verifiedOn", "status"]
};

router.use(authenticate, requirePermission(PERMISSIONS.CERTIFICATES_MANAGE));

router.get("/dashboard", async (req, res, next) => {
  try {
    const schoolId = requireSchool(req, res);
    if (!schoolId) return;
    const [certificates, transcripts, verifications, issued, published, valid] = await Promise.all([
      prisma.certificateRecord.count({ where: { schoolId } }),
      prisma.transcriptRecord.count({ where: { schoolId } }),
      prisma.certificateVerification.count({ where: { schoolId } }),
      prisma.certificateRecord.count({ where: { schoolId, status: "ISSUED" } }),
      prisma.transcriptRecord.count({ where: { schoolId, status: "PUBLISHED" } }),
      prisma.certificateVerification.count({ where: { schoolId, status: "VALID" } })
    ]);
    return ok(res, { certificates, transcripts, verifications, issued, published, valid });
  } catch (error) {
    next(error);
  }
});
...
router.post("/:resource", async (req, res, next) => {
  try {
    ...
    if (isPrismaError(error)) return fail(res, 409, "CONFLICT", "Certificate record could not be saved.");
    next(error);
  }
});
...
function parseResource(req: Request, res: Response): Resource | null {
  ...
  if (!resource || !(resource in schemas)) {
    fail(res, 404, "NOT_FOUND", "Certificates resource not found.");
    return null;
  }
  return resource;
}
...
function buildWhere(resource: Resource, schoolId: string, search?: string, status?: string) {
  ...
  const searchFields: Record<Resource, string[]> = {
    certificates: ["studentName", "certificateNumber", "title", "fileUrl"],
    transcripts: ["studentName", "transcriptNumber", "academicYear", "gpa", "fileUrl"],
    verifications: ["verificationCode", "certificateNumber", "studentName"]
  };
