...
const schemas = {
  students: z.object({ studentName: z.string().min(2), documentType: z.string().min(2), fileUrl: z.string().min(2), verifiedBy: z.string().optional(), uploadedOn: z.coerce.date(), status: z.string().default("PENDING") }),
  teachers: z.object({ teacherName: z.string().min(2), documentType: z.string().min(2), fileUrl: z.string().min(2), verifiedBy: z.string().optional(), uploadedOn: z.coerce.date(), status: z.string().default("PENDING") }),
  contracts: z.object({ partyName: z.string().min(2), contractType: z.string().min(2), fileUrl: z.string().min(2), effectiveOn: z.coerce.date(), expiresOn: z.coerce.date().optional(), status: z.string().default("ACTIVE") }),
  archive: z.object({ title: z.string().min(2), archiveType: z.string().min(2), fileUrl: z.string().min(2), archivedBy: z.string().min(2), archivedOn: z.coerce.date(), retentionTag: z.string().min(2), status: z.string().default("ARCHIVED") })
} as const;

type Resource = keyof typeof schemas;

const modelByResource: Record<Resource, keyof typeof prisma> = {
  students: "documentStudent",
  teachers: "documentTeacher",
  contracts: "documentContract",
  archive: "documentArchive"
};

const columnsByResource: Record<Resource, string[]> = {
  students: ["id", "studentName", "documentType", "fileUrl", "verifiedBy", "uploadedOn", "status"],
  teachers: ["id", "teacherName", "documentType", "fileUrl", "verifiedBy", "uploadedOn", "status"],
  contracts: ["id", "partyName", "contractType", "fileUrl", "effectiveOn", "expiresOn", "status"],
  archive: ["id", "title", "archiveType", "fileUrl", "archivedBy", "archivedOn", "retentionTag", "status"]
};

router.use(authenticate, requirePermission(PERMISSIONS.DOCUMENTS_MANAGE));

router.get("/dashboard", async (req, res, next) => {
  try {
    const schoolId = requireSchool(req, res);
    if (!schoolId) return;
    const [students, teachers, contracts, archive, verified, pending] = await Promise.all([
      prisma.documentStudent.count({ where: { schoolId } }),
      prisma.documentTeacher.count({ where: { schoolId } }),
      prisma.documentContract.count({ where: { schoolId } }),
      prisma.documentArchive.count({ where: { schoolId } }),
      Promise.all([
        prisma.documentStudent.count({ where: { schoolId, status: "VERIFIED" } }),
        prisma.documentTeacher.count({ where: { schoolId, status: "VERIFIED" } })
      ]).then((items) => items.reduce((sum, item) => sum + item, 0)),
      Promise.all([
        prisma.documentStudent.count({ where: { schoolId, status: "PENDING" } }),
        prisma.documentTeacher.count({ where: { schoolId, status: "PENDING" } })
      ]).then((items) => items.reduce((sum, item) => sum + item, 0))
    ]);
    return ok(res, { students, teachers, contracts, archive, verified, pending });
  } catch (error) {
    next(error);
  }
});
...
router.post("/:resource", async (req, res, next) => {
  try {
    ...
    if (isPrismaError(error)) return fail(res, 409, "CONFLICT", "Document record could not be saved.");
    next(error);
  }
});
...
function parseResource(req: Request, res: Response): Resource | null {
  ...
  if (!resource || !(resource in schemas)) {
    fail(res, 404, "NOT_FOUND", "Documents resource not found.");
    return null;
  }
  return resource;
}
...
function buildWhere(resource: Resource, schoolId: string, search?: string, status?: string) {
  ...
  const searchFields: Record<Resource, string[]> = {
    students: ["studentName", "documentType", "fileUrl", "verifiedBy"],
    teachers: ["teacherName", "documentType", "fileUrl", "verifiedBy"],
    contracts: ["partyName", "contractType", "fileUrl"],
    archive: ["title", "archiveType", "fileUrl", "archivedBy", "retentionTag"]
  };
...
function orderByFor(resource: Resource) {
  if (resource === "contracts") return { effectiveOn: "desc" };
  if (resource === "archive") return { archivedOn: "desc" };
  return { uploadedOn: "desc" };
}
